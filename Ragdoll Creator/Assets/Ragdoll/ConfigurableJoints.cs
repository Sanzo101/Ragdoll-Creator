...
public class ConfigurableJoints : MonoBehaviour
{
    ...
    // { Hips, Foot.R, Calf.R, Thigh.R, Foot.L, Calf.L, Thigh.L, Head, neck, Wrist.R(Hand), ForeArm.R, Arm.R, Wrist.L(Hand), ForeArm.L, Arm.L, Chest, LowerSpine} 17
    private float[] maxForceProfile = { 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,1f,1f,1f,1f,1f,1f };
    private float[] maxJointTorqueProfile = { 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,1f,1f,1f,1f,1f,1f };
    private float[] jointDampingProfile = { 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f, 1f,1f,1f,1f,1f,1f,1f};
    private bool[] limbProfile = { true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true }; // False means no force and torque will be applied to specified limb.
    ...
    void Start()
    {
        ...
        foreach (Transform t in slaveTransforms)
        {
            ...
            if (t.GetComponent<Rigidbody>() != null)
            {
                ...
                i++;
                t.gameObject.AddComponent<CollisionDetection>();
            }
            j++;
        }
    ...
    public void FollowAnimation()
    {
        if (!isAlive)
        {
            MasterObj.GetComponent<PlayerController>().WalkEnabled = false;
            SetJointTorque(0, 0);
            return;
        }
        else if(falling && isAlive)
        {
            MasterObj.GetComponent<PlayerController>().WalkEnabled = false;
            SetJointTorque(0,0);
            return;
